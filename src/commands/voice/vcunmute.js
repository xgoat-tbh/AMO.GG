...
export default {
  ...
  async execute(message, args, client) {
    try {
      ...
      if (!targets.length) {
        return message.reply({
          ...v2Payload(createV2Error(`${emojis.error} No valid targets to unmute.`, client)),
          allowedMentions: { repliedUser: false },
        });
      }
      ...
      await message.reply({
        ...v2Payload(createV2Success(`${emojis.success} Server unmuted **${targets.length}** member${targets.length !== 1 ? 's' : ''}.`, client)),
        allowedMentions: { repliedUser: false },
      });
    } catch (error) {
