...
export default {
  ...
  async execute(message, args, client) {
    try {
      ...
      if (!targets.length) {
        return message.reply({
          ...v2Payload(createV2Error('❌ No valid targets to undeafen.', client)),
          allowedMentions: { repliedUser: false },
        });
      }
      ...
      await message.reply({
        ...v2Payload(createV2Success(`✅ Server undeafened **${targets.length}** member${targets.length !== 1 ? 's' : ''}.`, client)),
        allowedMentions: { repliedUser: false },
      });
    } catch (error) {
