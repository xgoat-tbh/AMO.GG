...
export default {
  name: 'vcmute',
  aliases: ['vm'],
  description: 'Server mute a user or everyone in your VC.',
  usage: '?vcmute <@user | all>',
  ...
  async execute(message, args, client) {
    try {
      ...
      if (!targets.length) {
        return message.reply({
          ...v2Payload(createV2Error(`${emojis.error} No valid targets to mute.`, client)),
          allowedMentions: { repliedUser: false },
        });
      }

      await voiceManager.muteMembers(targets, true);
      ...
      await message.reply({
        ...v2Payload(createV2Success(`${emojis.success} Server muted **${targets.length}** member${targets.length !== 1 ? 's' : ''}.`, client)),
        allowedMentions: { repliedUser: false },
      });
    } catch (error) {
