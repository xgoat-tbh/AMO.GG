...
export default {
  ...
  async execute(interaction, client) {
    ...
    try {
      // Add Connect override for the target user
      await channel.permissionOverwrites.edit(targetUserId, {
        Connect: true,
      });

      const successContainer = createV2Success(
        `${emojis.success} Successfully trusted <@${targetUserId}>! They can now join this voice channel even when it is locked.`,
        client
      );
      ...
      logger.error('GPVC_TRUST', `Failed to trust member: ${error.message}`);
      const errorContainer = createV2Error(
        `${emojis.error} Failed to trust member: ${error.message}`,
        client
      );
