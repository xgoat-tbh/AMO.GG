...
export default {
  ...
  async execute(interaction, client) {
    ...
    try {
      const db = getDb();
      ConfigRepo.set(db, settingKey, channelId);
      ...
      await interaction.followUp({
        content: `${emojis.success} Successfully updated setting \`${settingKey}\` to <#${channelId}>.`,
        flags: MessageFlags.Ephemeral,
      });

      logger.info('CONFIG', `Admin ${interaction.user.id} updated config "${settingKey}" to channel "${channelId}"`);
    } catch (error) {
      logger.error('CONFIG_CHANNEL_SET', `Failed to save configuration override: ${error.message}`, error);
      await interaction.reply({
