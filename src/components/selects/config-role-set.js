...
export default {
  ...
  async execute(interaction, client) {
    ...
    try {
      const db = getDb();
      ConfigRepo.set(db, settingKey, roleId);
      ...
      await interaction.followUp({
        content: `${emojis.success} Successfully updated setting \`${settingKey}\` to <@&${roleId}>.`,
        flags: MessageFlags.Ephemeral,
      });

      logger.info('CONFIG', `Admin ${interaction.user.id} updated config "${settingKey}" to role "${roleId}"`);
    } catch (error) {
      logger.error('CONFIG_ROLE_SET', `Failed to save configuration override: ${error.message}`, error);
      await interaction.reply({
